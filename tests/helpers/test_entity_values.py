"""Test the entity values helper."""

from collections import OrderedDict

from tryke import expect, test

from homeassistant.helpers.entity_values import EntityValues as EV

ent = "test.test"


@test
def override_single_value() -> None:
    """Test values with exact match."""
    store = EV({ent: {"key": "value"}})
    store.get.cache_clear()
    expect(store.get(ent)).to_equal({"key": "value"})
    expect(store.get.cache_info().currsize).to_equal(1)
    expect(store.get.cache_info().misses).to_equal(1)
    expect(store.get(ent)).to_equal({"key": "value"})
    expect(store.get.cache_info().currsize).to_equal(1)
    expect(store.get.cache_info().misses).to_equal(1)
    expect(store.get.cache_info().hits).to_equal(1)


@test
def override_by_domain() -> None:
    """Test values with domain match."""
    store = EV(domain={"test": {"key": "value"}})
    expect(store.get(ent)).to_equal({"key": "value"})


@test
def override_by_glob() -> None:
    """Test values with glob match."""
    store = EV(glob={"test.?e*": {"key": "value"}})
    expect(store.get(ent)).to_equal({"key": "value"})


@test
def glob_overrules_domain() -> None:
    """Test domain overrules glob match."""
    store = EV(domain={"test": {"key": "domain"}}, glob={"test.?e*": {"key": "glob"}})
    expect(store.get(ent)).to_equal({"key": "glob"})


@test
def exact_overrules_domain() -> None:
    """Test exact overrules domain match."""
    store = EV(
        exact={"test.test": {"key": "exact"}},
        domain={"test": {"key": "domain"}},
        glob={"test.?e*": {"key": "glob"}},
    )
    expect(store.get(ent)).to_equal({"key": "exact"})


@test
def merging_values() -> None:
    """Test merging glob, domain and exact configs."""
    store = EV(
        exact={"test.test": {"exact_key": "exact"}},
        domain={"test": {"domain_key": "domain"}},
        glob={"test.?e*": {"glob_key": "glob"}},
    )
    expect(store.get(ent)).to_equal(
        {
            "exact_key": "exact",
            "domain_key": "domain",
            "glob_key": "glob",
        }
    )


@test
def glob_order() -> None:
    """Test merging glob, domain and exact configs."""
    glob = OrderedDict()
    glob["test.*est"] = {"value": "first"}
    glob["test.*"] = {"value": "second"}

    store = EV(glob=glob)
    expect(store.get(ent)).to_equal({"value": "second"})
