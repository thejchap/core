...
def _create_core_files_yaml(base_platforms: list[str]) -> dict[str, Any]:
...
def skip_specific_integrations() -> None:
    ...
    validate({}, config)
    expect(config.errors).to_equal([])


@test
def valid_alignment(config: Config = Depends(config)) -> None:
...
@test
def missing_entity_platform(config: Config = Depends(config)) -> None:
...
@test
def unexpected_entry(config: Config = Depends(config)) -> None:
