"""Tryke fixtures for IRM KMI integration tests."""
...
def mock_get_forecast_in_benelux() -> Generator[None]:
    """Mock IrmKmiApiClient.get_forecasts_coord() to return a valid Benelux location."""
    with patch(
...
def mock_get_forecast_out_benelux_then_in_belgium() -> Generator[None]:
    """Mock forecast returning outside benelux, then inside belgium."""
    with patch(
        "homeassistant.components.irm_kmi.config_flow.IrmKmiApiClient.get_forecasts_coord",
        side_effect=[
