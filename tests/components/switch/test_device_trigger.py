"""The test for switch device automation (tryke port)."""
...
async def if_fires_on_state_change(
    ...
    expect({service_calls[0].data["some"], service_calls[1].data["some"]}).to_equal(
        {
    ...
    expect({service_calls[2].data["some"], service_calls[3].data["some"]}).to_equal(
        {
