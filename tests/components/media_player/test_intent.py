"""The tests for the media_player platform (tryke port)."""
...
@test
async def pause_media_player_intent(
    ...
    response = await intent.async_handle(
        hass, "test", media_player_intent.INTENT_MEDIA_PAUSE
    )
    ...
    await _expect_raises(
        ...
        intent.async_handle(hass, "test", media_player_intent.INTENT_MEDIA_PAUSE),
    )
...
@test
async def unpause_media_player_intent(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    ...
    response = await intent.async_handle(
        hass, "test", media_player_intent.INTENT_MEDIA_UNPAUSE
    )
    await hass.async_block_till_done()
...
@test
async def next_media_player_intent(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    ...
    response = await intent.async_handle(
        hass, "test", media_player_intent.INTENT_MEDIA_NEXT
    )
    await hass.async_block_till_done()
    ...
    await _expect_raises(
        intent.MatchFailedError,
        intent.async_handle(hass, "test", media_player_intent.INTENT_MEDIA_NEXT),
    )
...
@test
async def previous_media_player_intent(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    ...
    response = await intent.async_handle(
        hass, "test", media_player_intent.INTENT_MEDIA_PREVIOUS
    )
    await hass.async_block_till_done()
    ...
    await _expect_raises(
        intent.MatchFailedError,
        intent.async_handle(hass, "test", media_player_intent.INTENT_MEDIA_PREVIOUS),
    )
...
@test
async def volume_media_player_intent(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
...
@test
async def media_player_mute_intent(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    ...
    response = await intent.async_handle(
        hass, "test", media_player_intent.INTENT_PLAYER_MUTE, {}
    )
    await hass.async_block_till_done()
...
@test
async def media_player_unmute_intent(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    """Test HassMediaPlayerMute (unmute) intent for media players."""
    await media_player_intent.async_setup_intents(hass)
    ...
    response = await intent.async_handle(
        hass, "test", media_player_intent.INTENT_PLAYER_UNMUTE, {}
    )
    await hass.async_block_till_done()
...
@test
async def multiple_media_players(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    ...
    hass.states.async_set(
        living_room_tv.entity_id, STATE_PAUSED, attributes=attributes
    )
...
@test
async def manual_pause_unpause(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fixture),
) -> None:
    ...
    device_1 = entity_registry.async_update_entity(
        device_1.entity_id, name="device 1"
    )
    ...
    device_2 = entity_registry.async_update_entity(
        device_2.entity_id, name="device 2"
    )
    ...
    # Unpause with no constraints — should resume the more recently (manually)
    # paused device.
    context = Context()
    calls = async_mock_service(hass, DOMAIN, SERVICE_MEDIA_PLAY)
    response = await intent.async_handle(
...
@test
async def search_and_play_media_player_intent(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    ...
    hass.states.async_set(entity_id, STATE_IDLE, attributes={})
    await _expect_raises(
        intent.MatchFailedError,
        intent.async_handle(
            hass,
    ...
    # Feature not supported (missing SEARCH_MEDIA)
    hass.states.async_set(
        entity_id,
        STATE_IDLE,
...
@test
async def search_and_play_media_player_intent_with_media_class(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    ...
    # Invalid media_class should raise validation error
    await _expect_raises(
...
# Parametrized over volume direction ("up", "down") + EntityComponent harness.
# Skip until Tryke parametrize + MockEntityPlatform helpers land.
@test.skip("requires pytest parametrize over volume direction + EntityComponent")
async def volume_relative_media_player_intent() -> None:
    """Skipped: requires parametrize + MockEntityPlatform setup."""
