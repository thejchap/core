...
Mirrors the heavy SSDP / remote-control mock chain from the upstream
``conftest.py``. All "autouse" fixtures from the pytest version are
re-exported here as ordinary tryke fixtures; per-test files wire them in
via a module-local ``_trigger_executor`` to keep the autouse semantics.
"""
...
@fixture
def app_list_delay() -> Generator[None]:
...
@fixture
def mac_address() -> Generator[Mock]:
...
@fixture
def upnp_factory() -> Generator[Mock]:
    """Patch UpnpFactory."""
    with patch(
...
@fixture
def upnp_device(
    upnp_factory_value: Mock = Depends(upnp_factory),
) -> Mock:
    """Patch async_upnp_client."""
    upnp_device = Mock(UpnpDevice)
    upnp_device.services = {}
    upnp_factory_value.async_create_device.side_effect = [upnp_device]
    return upnp_device


@fixture
def dmr_device(
    _upnp: Mock = Depends(upnp_device),
) -> Generator[Mock]:
    """Patch async_upnp_client."""
    with patch(
...
@fixture
def remote_legacy() -> Generator[Mock]:
...
@fixture
def rest_api_failing() -> Generator[None]:
    """Patch the samsungtvws SamsungTVAsyncRest to fail."""
    with patch(
        "homeassistant.components.samsungtv.bridge.SamsungTVAsyncRest",
        autospec=True,
    ) as rest_api_class:
...
@fixture
def remote_encrypted_websocket_failing() -> Generator[None]:
    """Patch the samsungtvws SamsungTVEncryptedWSAsyncRemote to fail."""
    with patch(
...
@fixture
def remote_websocket() -> Generator[Mock]:
...
@fixture
def remote_encrypted_websocket() -> Generator[Mock]:
