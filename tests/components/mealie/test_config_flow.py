...
from ._fixtures import mock_config_entry, mock_mealie_client, mock_setup_entry
...
async def full_flow(
    ...
    mock_mealie_client: AsyncMock = Depends(mock_mealie_client),
...
@test.cases(
    test.case("connection_error", exception=MealieConnectionError, error="cannot_connect"),
    test.case("auth_error", exception=MealieAuthenticationError, error="invalid_auth"),
    test.case("unknown_error", exception=Exception, error="unknown"),
)
...
@test.cases(
    test.case("v1_0_0_beta_5", version="v1.0.0beta-5"),
    test.case("v1_0_0_RC2", version="v1.0.0-RC2"),
    test.case("v0_1_0", version="v0.1.0"),
    test.case("v1_9_0", version="v1.9.0"),
    test.case("v2_0_0_beta_2", version="v2.0.0beta-2"),
)
...
async def reauth_flow(
    ...
    expect(mock_config_entry.data[CONF_API_TOKEN]).to_equal("token2")
...
async def reconfigure_flow(
    ...
    expect(mock_config_entry.data[CONF_API_TOKEN]).to_equal("token2")
    expect(mock_config_entry.data[CONF_HOST]).to_equal("http://test:9090")
    expect(mock_config_entry.data[CONF_VERIFY_SSL]).to_be(False)
