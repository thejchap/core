...
from ndms2_client.client import InterfaceInfo
...
from ._fixtures import connect, connect_error
...
async def connection_error(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    _err: None = Depends(connect_error),
) -> None:
...
@test
async def options_not_initialized(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
...
@test
async def options_connection_error(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    """Test updating options."""
...
@test
async def options_interface_filter(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    ...
    expect(isinstance(interfaces_schema, vol.Required)).to_be_truthy()
...
@test
async def ssdp_reject_non_keenetic(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
