"""Tryke skip stub for test_binary_sensor.py: uses syrupy snapshot — needs pytest --snapshot-update first."""

from tryke import expect, test


@test
def module_importable() -> None:
    """Smoke test: the homeassistant.components.august.binary_sensor module imports cleanly."""
    from homeassistant.components.august import binary_sensor  # noqa: PLC0415
    expect(binary_sensor).not_.to_be(None)


@test.skip("uses syrupy snapshot — needs pytest --snapshot-update first")
async def doorsense() -> None:
    """Stub for test_doorsense."""


@test.skip("uses syrupy snapshot — needs pytest --snapshot-update first")
async def lock_bridge_offline() -> None:
    """Stub for test_lock_bridge_offline."""


@test.skip("uses syrupy snapshot — needs pytest --snapshot-update first")
async def create_doorbell() -> None:
    """Stub for test_create_doorbell."""


@test.skip("uses syrupy snapshot — needs pytest --snapshot-update first")
async def create_doorbell_offline() -> None:
    """Stub for test_create_doorbell_offline."""


@test.skip("uses syrupy snapshot — needs pytest --snapshot-update first")
async def create_doorbell_with_motion() -> None:
    """Stub for test_create_doorbell_with_motion."""


@test.skip("uses syrupy snapshot — needs pytest --snapshot-update first")
async def doorbell_update_via_pubnub() -> None:
    """Stub for test_doorbell_update_via_pubnub."""


@test.skip("uses syrupy snapshot — needs pytest --snapshot-update first")
async def doorbell_device_registry() -> None:
    """Stub for test_doorbell_device_registry."""


@test.skip("uses syrupy snapshot — needs pytest --snapshot-update first")
async def door_sense_update_via_pubnub() -> None:
    """Stub for test_door_sense_update_via_pubnub."""


@test.skip("uses syrupy snapshot — needs pytest --snapshot-update first")
async def create_lock_with_doorbell() -> None:
    """Stub for test_create_lock_with_doorbell."""


