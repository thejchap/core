...
@test
async def august_api_is_failing(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fx),
) -> None:
    """Config entry state is SETUP_RETRY when august api is failing."""
    config_entry, _pubnub = await _create_august_with_devices(
        hass,
...
@test
async def august_late_auth_failure(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fx),
) -> None:
...
@test
async def unlock_throws_august_api_http_error(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fx),
) -> None:
...
@test
async def lock_throws_august_api_http_error(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fx),
) -> None:
...
@test
async def brand_migration_issue(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fx),
) -> None:
    ...
    expect(issue_reg.async_get_issue(DOMAIN, "yale_brand_migration")).to_be(None)


@test
async def oauth_migration_on_legacy_entry(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fx),
) -> None:
    ...
    expect(legacy_entry.state).to_be(ConfigEntryState.SETUP_ERROR)
