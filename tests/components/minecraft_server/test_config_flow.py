...
async def full_flow_legacy_java(
    ...
    with (
        ...
        expect(result["title"]).to_equal(USER_INPUT[CONF_ADDRESS])
        expect(result["data"][CONF_ADDRESS]).to_equal(TEST_ADDRESS)
        expect(result["data"][CONF_TYPE]).to_equal(MinecraftServerType.LEGACY_JAVA_EDITION)
...
async def service_already_configured_bedrock(
    ...
    """Test config flow abort if a Bedrock server is already configured."""
...
async def service_already_configured_legacy_java(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    legacy_java_mock_config_entry: MockConfigEntry = Depends(legacy_java_mock_config_entry),
) -> None:
...
async def recovery_bedrock(
    ...
    with (
        ...
        expect(result2["title"]).to_equal(USER_INPUT[CONF_ADDRESS])
        expect(result2["data"][CONF_ADDRESS]).to_equal(TEST_ADDRESS)
        expect(result2["data"][CONF_TYPE]).to_equal(MinecraftServerType.BEDROCK_EDITION)
...
async def recovery_legacy_java(
    ...
    with (
        ...
        expect(result2["title"]).to_equal(USER_INPUT[CONF_ADDRESS])
        expect(result2["data"][CONF_ADDRESS]).to_equal(TEST_ADDRESS)
        expect(result2["data"][CONF_TYPE]).to_equal(MinecraftServerType.LEGACY_JAVA_EDITION)
