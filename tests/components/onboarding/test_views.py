...
from ._fixtures import auth_active, mock_default_integrations, no_rpi
...
async def onboarding_progress(
    ...
    _auth_active: None = Depends(auth_active),
...
@test
async def onboarding_user_already_done(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test
async def onboarding_user_invalid_name(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test
async def onboarding_user_race(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test
async def onboarding_integration(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test
async def onboarding_integration_missing_credential(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test
async def onboarding_integration_invalid_redirect_uri(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test
async def onboarding_integration_requires_auth(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test.skip("optional deps not installed: rpi_bad_power, metno, radios, gtts")
@test.cases(
    test.case("met", domain="met"),
    test.case("shopping_list", domain="shopping_list"),
    test.case("google_translate", domain="google_translate"),
    test.case("radio_browser", domain="radio_browser"),
)
async def onboarding_core_sets_up_default_integrations(
    domain: str,
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
    hass_client: ClientSessionGenerator = Depends(hass_client_fixture),
    _mock_default_integrations: None = Depends(mock_default_integrations),
) -> None:
    """Test finishing the core step sets up default integrations."""
    mock_storage(hass_storage, {"done": [const.STEP_USER]})
...
@test.skip("optional dep not installed: rpi_bad_power")
async def onboarding_core_no_rpi_power(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
    hass_client: ClientSessionGenerator = Depends(hass_client_fixture),
    _no_rpi: None = Depends(no_rpi),
    _mock_default_integrations: None = Depends(mock_default_integrations),
) -> None:
...
@test.skip("optional dep not installed: rpi_bad_power")
async def onboarding_core_ensures_analytics_loaded(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test
async def onboarding_installation_type(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test.cases(
    test.case("installation_type", method="get", view="installation_type", kwargs={}),
)
async def onboarding_view_after_done(
    method: str,
    view: str,
    kwargs: dict[str, Any],
    _executor: int = Depends(_trigger_executor),
...
@test.skip("optional deps not installed: metno, radios, gtts")
...
@test.cases(
    test.case(
        "onboarding", domain="onboarding", expected_result={"integration_loaded": True}
    ),
    test.case(
        "non_existing_domain",
        domain="non_existing_domain",
        expected_result={"integration_loaded": False},
    ),
)
async def wait_integration(
    domain: str,
...
@test
async def wait_integration_startup(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test
async def not_setup_platform_if_onboarded(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test
async def setup_platform_if_not_onboarded(
    _executor: int = Depends(_trigger_executor),
    _auth_active: None = Depends(auth_active),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
...
@test.cases(
    test.case(
        "missing_method",
        platform_mock=Mock(some_method=AsyncMock(), spec=["some_method"]),
    ),
    test.case("empty_spec", platform_mock=Mock(spec=[])),
)
