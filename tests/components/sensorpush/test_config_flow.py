...
async def async_step_bluetooth_valid_device(
    ...
    expect(result2["title"]).to_equal("HTP.xw F4D")
    expect(result2["data"]).to_equal({})
    expect(result2["result"].unique_id).to_equal(
        "4125DDBA-2774-4851-9889-6AADDD4CAC3D"
    )


@test
async def async_step_bluetooth_not_sensorpush(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
...
async def async_step_user_with_found_devices(
    ...
    expect(result2["type"]).to_be(FlowResultType.CREATE_ENTRY)
    expect(result2["title"]).to_equal("HT.w 0CA1")
    expect(result2["data"]).to_equal({})
    expect(result2["result"].unique_id).to_equal(
...
async def async_step_user_replace_ignored(
    ...
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=HTW_SERVICE_INFO.address,
...
async def async_step_user_takes_precedence_over_discovery(
    ...
    expect(hass.config_entries.flow.async_progress(DOMAIN)).to_equal([])
