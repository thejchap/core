...
@test.cases(
    test.case("opened", trigger_key="door.opened"),
    test.case("closed", trigger_key="door.closed"),
)
async def door_triggers_gated_by_labs_flag(
    trigger_key: str,
...
@test.cases(
    ...
    test.case(
        "opened",
        trigger_key="door.opened",
        base_options={},
        supports_behavior=True,
        supports_duration=True,
    ),
)
async def door_trigger_options_validation(
    trigger_key: str,
...
@test
async def door_trigger_binary_sensor_behavior_any(
    _executor: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    _labs: None = Depends(enable_labs_preview_features),
...
@test
async def door_trigger_cover_behavior_any(
    _executor: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    _labs: None = Depends(enable_labs_preview_features),
...
@test
async def door_trigger_binary_sensor_behavior_first(
    _executor: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    _labs: None = Depends(enable_labs_preview_features),
    target_binary_sensors: dict[str, list[str]] = Depends(target_binary_sensors_fixture),
...
@test
async def door_trigger_binary_sensor_behavior_last(
    _executor: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    _labs: None = Depends(enable_labs_preview_features),
    target_binary_sensors: dict[str, list[str]] = Depends(target_binary_sensors_fixture),
...
@test
async def door_trigger_cover_behavior_first(
    _executor: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    _labs: None = Depends(enable_labs_preview_features),
    target_covers: dict[str, list[str]] = Depends(target_covers_fixture),
...
@test
async def door_trigger_cover_behavior_last(
    _executor: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    _labs: None = Depends(enable_labs_preview_features),
    target_covers: dict[str, list[str]] = Depends(target_covers_fixture),
