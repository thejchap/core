...
from ._fixtures import (
    register_hci0_scanner as register_hci0_scanner_fixture,
    register_hci1_scanner as register_hci1_scanner_fixture,
    register_non_connectable_scanner as register_non_connectable_scanner_fixture,
)
...
@test
async def subscribe_advertisements(
    hass: HomeAssistant = Depends(hass_fixture),
    _enable_bluetooth: None = Depends(enable_bluetooth_fixture),
    _register_hci0_scanner: None = Depends(register_hci0_scanner_fixture),
    _register_hci1_scanner: None = Depends(register_hci1_scanner_fixture),
    ...
    expect(new_time > adv_time).to_be_truthy()
    ...
    async with asyncio.timeout(1):
        response = await client.receive_json()
    expect(response["event"]).to_equal({"remove": [{"address": "44:44:33:11:23:12"}]})


@test
async def subscribe_connection_allocations(
    hass: HomeAssistant = Depends(hass_fixture),
    _enable_bluetooth: None = Depends(enable_bluetooth_fixture),
    _register_hci0_scanner: None = Depends(register_hci0_scanner_fixture),
    _register_hci1_scanner: None = Depends(register_hci1_scanner_fixture),
    _register_non_connectable_scanner: None = Depends(
        register_non_connectable_scanner_fixture
    ),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
...
@test
async def subscribe_connection_allocations_specific_scanner(
    hass: HomeAssistant = Depends(hass_fixture),
    _enable_bluetooth: None = Depends(enable_bluetooth_fixture),
    _register_non_connectable_scanner: None = Depends(
        register_non_connectable_scanner_fixture
...
@test
async def subscribe_connection_allocations_invalid_config_entry_id(
    hass: HomeAssistant = Depends(hass_fixture),
    _enable_bluetooth: None = Depends(enable_bluetooth_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
    ...
    expect(response["error"]["code"]).to_equal("invalid_config_entry_id")
    expect(response["error"]["message"]).to_equal("Config entry non_existent not found")


@test
async def subscribe_connection_allocations_invalid_scanner(
    hass: HomeAssistant = Depends(hass_fixture),
    _enable_bluetooth: None = Depends(enable_bluetooth_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
    ...
    async with asyncio.timeout(1):
        response = await client.receive_json()
    expect(response["success"]).to_be_falsy()
    expect(response["error"]["code"]).to_equal("invalid_source")
    expect(response["error"]["message"]).to_equal("Source invalid not found")


@test
async def subscribe_scanner_details(
    hass: HomeAssistant = Depends(hass_fixture),
    _enable_bluetooth: None = Depends(enable_bluetooth_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
...
@test
async def subscribe_scanner_details_specific_scanner(
    hass: HomeAssistant = Depends(hass_fixture),
    _enable_bluetooth: None = Depends(enable_bluetooth_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
...
@test
async def subscribe_scanner_details_invalid_config_entry_id(
    hass: HomeAssistant = Depends(hass_fixture),
    _enable_bluetooth: None = Depends(enable_bluetooth_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
...
@test
async def subscribe_scanner_state(
    hass: HomeAssistant = Depends(hass_fixture),
    _enable_bluetooth: None = Depends(enable_bluetooth_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
...
@test
async def subscribe_scanner_state_specific_scanner(
    hass: HomeAssistant = Depends(hass_fixture),
    _enable_bluetooth: None = Depends(enable_bluetooth_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
...
@test
async def subscribe_scanner_state_invalid_config_entry_id(
    hass: HomeAssistant = Depends(hass_fixture),
    _enable_bluetooth: None = Depends(enable_bluetooth_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
