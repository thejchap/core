"""Tryke fixtures for the SystemNexa2 integration."""
...
@fixture
def device_info(is_dimmable: bool = Depends(dimmable)) -> dict[str, Any]:
    ...
    return {
        "name": "In-Wall Dimmer" if is_dimmable else "Outdoor Smart Plug",
        "model": "WBD-01" if is_dimmable else "WPO-01",
        "unique_id": "aabbccddee01" if is_dimmable else "aabbccddee02",
        "host": "10.0.0.101" if is_dimmable else "10.0.0.100",
        "initial_state": 0.5 if is_dimmable else 1.0,
...
def mock_system_nexa_2_device(
    ...
    with (
        ...
        device.info_data = InformationData(
            ...
            dimmable=info["dimmable"],
        )

        device.settings = info["settings"]
        ...
        async def mock_connect():
            if mock_device.initiate_device.call_args:
                ...
                if on_update:
                    await on_update(StateChange(state=info["initial_state"]))
...
@fixture
def mock_patch_get_host() -> Generator[MagicMock]:
...
@fixture
def mock_zeroconf_discovery_info() -> ZeroconfServiceInfo:
