"""Test Home Assistant exposed entities helper."""

from syrupy.assertion import SnapshotAssertion
from tryke import Depends, expect, fixture, test

from homeassistant.components.homeassistant.exposed_entities import (
    DATA_EXPOSED_ENTITIES,
    ExposedEntities,
    ExposedEntity,
    async_expose_entity,
    async_get_assistant_settings,
    async_get_entity_settings,
    async_listen_entity_updates,
    async_should_expose,
)
from homeassistant.const import CLOUD_NEVER_EXPOSED_ENTITIES, EntityCategory
from homeassistant.core import HomeAssistant
from homeassistant.exceptions import HomeAssistantError
from homeassistant.helpers import entity_registry as er
from homeassistant.setup import async_setup_component

from tests.common import flush_store
from tests.hass_fixtures import (
    entity_registry as entity_registry_fx,
    hass as hass_fx,
    hass_ws_client as hass_ws_client_fx,
    mock_network,
)
from tests.hass_tryke_helpers import snapshot
from tests.typing import WebSocketGenerator


@fixture
def _trigger_executor(_network: None = Depends(mock_network)) -> int:
    """Opt into Tryke's HookExecutor path."""
    return 0


def _entities_unique_id(entity_registry: er.EntityRegistry) -> dict[str, str]:
    """Create some entities in the entity registry."""
    entry_blocked = entity_registry.async_get_or_create(
        "group", "test", "unique", suggested_object_id="all_locks"
    )
    expect(entry_blocked.entity_id).to_equal(CLOUD_NEVER_EXPOSED_ENTITIES[0])
    entry_lock = entity_registry.async_get_or_create("lock", "test", "unique1")
    entry_binary_sensor = entity_registry.async_get_or_create(
        "binary_sensor", "test", "unique1"
    )
    entry_binary_sensor_door = entity_registry.async_get_or_create(
        "binary_sensor",
        "test",
        "unique2",
        original_device_class="door",
    )
    entry_sensor = entity_registry.async_get_or_create("sensor", "test", "unique1")
    entry_sensor_temperature = entity_registry.async_get_or_create(
        "sensor",
        "test",
        "unique3",
        original_device_class="temperature",
    )
    entry_media_player = entity_registry.async_get_or_create(
        "media_player", "test", "unique4", original_device_class="media_player"
    )
    return {
        "blocked": entry_blocked.entity_id,
        "lock": entry_lock.entity_id,
        "binary_sensor": entry_binary_sensor.entity_id,
        "door_sensor": entry_binary_sensor_door.entity_id,
        "sensor": entry_sensor.entity_id,
        "temperature_sensor": entry_sensor_temperature.entity_id,
        "media_player": entry_media_player.entity_id,
    }


def _entities_no_unique_id(hass: HomeAssistant) -> dict[str, str]:
    """Create some entities not in the entity registry."""
    blocked = CLOUD_NEVER_EXPOSED_ENTITIES[0]
    lock = "lock.test"
    binary_sensor = "binary_sensor.test"
    door_sensor = "binary_sensor.door"
    sensor = "sensor.test"
    sensor_temperature = "sensor.temperature"
    media_player = "media_player.test"
    hass.states.async_set(binary_sensor, "on", {})
    hass.states.async_set(door_sensor, "on", {"device_class": "door"})
    hass.states.async_set(sensor, "on", {})
    hass.states.async_set(sensor_temperature, "on", {"device_class": "temperature"})
    hass.states.async_set(media_player, "idle", {})
    return {
        "blocked": blocked,
        "lock": lock,
        "binary_sensor": binary_sensor,
        "door_sensor": door_sensor,
        "sensor": sensor,
        "temperature_sensor": sensor_temperature,
        "media_player": media_player,
    }


@test
async def load_preferences(hass: HomeAssistant = Depends(hass_fx)) -> None:
    """Make sure that we can load/save data correctly."""
    expect(await async_setup_component(hass, "homeassistant", {})).to_be_truthy()

    exposed_entities = hass.data[DATA_EXPOSED_ENTITIES]
    expect(exposed_entities._assistants).to_equal({})
    expect(exposed_entities.entities).to_equal({})

    exposed_entities.async_set_expose_new_entities("test1", True)
    exposed_entities.async_set_expose_new_entities("test2", False)

    async_expose_entity(hass, "test1", "light.kitchen", True)
    async_expose_entity(hass, "test1", "light.living_room", True)
    async_expose_entity(hass, "test2", "light.kitchen", True)
    async_expose_entity(hass, "test2", "light.kitchen", True)

    expect(list(exposed_entities._assistants)).to_equal(["test1", "test2"])
    expect(list(exposed_entities.entities)).to_equal(
        ["light.kitchen", "light.living_room"]
    )

    await flush_store(exposed_entities._store)

    exposed_entities2 = ExposedEntities(hass)
    await exposed_entities2.async_initialize()

    expect(exposed_entities._assistants).to_equal(exposed_entities2._assistants)
    expect(exposed_entities.entities).to_equal(exposed_entities2.entities)


@test
async def expose_entity(
    hass: HomeAssistant = Depends(hass_fx),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fx),
    hass_ws_client: WebSocketGenerator = Depends(hass_ws_client_fx),
) -> None:
    """Test expose entity."""
    ws_client = await hass_ws_client(hass)
    expect(await async_setup_component(hass, "homeassistant", {})).to_be_truthy()
    await hass.async_block_till_done()

    entry1 = entity_registry.async_get_or_create("test", "test", "unique1")
    entry2 = entity_registry.async_get_or_create("test", "test", "unique2")

    exposed_entities = hass.data[DATA_EXPOSED_ENTITIES]
    expect(len(exposed_entities.entities)).to_equal(0)

    # Set options
    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_entity",
            "assistants": ["cloud.alexa"],
            "entity_ids": [entry1.entity_id],
            "should_expose": True,
        }
    )

    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()

    entry1 = entity_registry.async_get(entry1.entity_id)
    expect(entry1.options).to_equal({"cloud.alexa": {"should_expose": True}})
    entry2 = entity_registry.async_get(entry2.entity_id)
    expect(entry2.options).to_equal({})
    expect(len(exposed_entities.entities)).to_equal(0)

    # Update options
    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_entity",
            "assistants": ["cloud.alexa", "cloud.google_assistant"],
            "entity_ids": [entry1.entity_id, entry2.entity_id],
            "should_expose": False,
        }
    )

    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()

    entry1 = entity_registry.async_get(entry1.entity_id)
    expect(entry1.options).to_equal(
        {
            "cloud.alexa": {"should_expose": False},
            "cloud.google_assistant": {"should_expose": False},
        }
    )
    entry2 = entity_registry.async_get(entry2.entity_id)
    expect(entry2.options).to_equal(
        {
            "cloud.alexa": {"should_expose": False},
            "cloud.google_assistant": {"should_expose": False},
        }
    )
    expect(len(exposed_entities.entities)).to_equal(0)


@test
async def expose_entity_unknown(
    hass: HomeAssistant = Depends(hass_fx),
    hass_ws_client: WebSocketGenerator = Depends(hass_ws_client_fx),
) -> None:
    """Test behavior when exposing an unknown entity."""
    ws_client = await hass_ws_client(hass)
    expect(await async_setup_component(hass, "homeassistant", {})).to_be_truthy()
    await hass.async_block_till_done()

    exposed_entities = hass.data[DATA_EXPOSED_ENTITIES]
    expect(len(exposed_entities.entities)).to_equal(0)

    # Set options
    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_entity",
            "assistants": ["cloud.alexa"],
            "entity_ids": ["test.test"],
            "should_expose": True,
        }
    )

    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()

    expect(len(exposed_entities.entities)).to_equal(1)
    expect(exposed_entities.entities).to_equal(
        {"test.test": ExposedEntity({"cloud.alexa": {"should_expose": True}})}
    )

    # Update options
    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_entity",
            "assistants": ["cloud.alexa", "cloud.google_assistant"],
            "entity_ids": ["test.test", "test.test2"],
            "should_expose": False,
        }
    )

    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()

    expect(len(exposed_entities.entities)).to_equal(2)
    expect(exposed_entities.entities).to_equal(
        {
            "test.test": ExposedEntity(
                {
                    "cloud.alexa": {"should_expose": False},
                    "cloud.google_assistant": {"should_expose": False},
                }
            ),
            "test.test2": ExposedEntity(
                {
                    "cloud.alexa": {"should_expose": False},
                    "cloud.google_assistant": {"should_expose": False},
                }
            ),
        }
    )


@test
async def expose_entity_blocked(
    hass: HomeAssistant = Depends(hass_fx),
    hass_ws_client: WebSocketGenerator = Depends(hass_ws_client_fx),
) -> None:
    """Test behavior when exposing a blocked entity."""
    ws_client = await hass_ws_client(hass)
    expect(await async_setup_component(hass, "homeassistant", {})).to_be_truthy()
    await hass.async_block_till_done()

    # Set options
    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_entity",
            "assistants": ["cloud.alexa"],
            "entity_ids": ["group.all_locks"],
            "should_expose": True,
        }
    )

    response = await ws_client.receive_json()
    expect(response["success"]).to_be_falsy()
    expect(response["error"]).to_equal(
        {
            "code": "not_allowed",
            "message": "can't expose 'group.all_locks'",
        }
    )


@test.cases(
    test.case("expose_new_true", expose_new=True),
    test.case("expose_new_false", expose_new=False),
)
async def expose_new_entities(
    expose_new: bool,
    hass: HomeAssistant = Depends(hass_fx),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fx),
    hass_ws_client: WebSocketGenerator = Depends(hass_ws_client_fx),
) -> None:
    """Test expose entity."""
    ws_client = await hass_ws_client(hass)
    expect(await async_setup_component(hass, "homeassistant", {})).to_be_truthy()
    await hass.async_block_till_done()

    entry1 = entity_registry.async_get_or_create("climate", "test", "unique1")
    entry2 = entity_registry.async_get_or_create("climate", "test", "unique2")

    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_new_entities/get",
            "assistant": "cloud.alexa",
        }
    )
    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()
    expect(response["result"]).to_equal({"expose_new": False})

    # Check if exposed - should be False
    expect(async_should_expose(hass, "cloud.alexa", entry1.entity_id)).to_be(False)

    # Expose new entities to Alexa
    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_new_entities/set",
            "assistant": "cloud.alexa",
            "expose_new": expose_new,
        }
    )
    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()
    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_new_entities/get",
            "assistant": "cloud.alexa",
        }
    )
    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()
    expect(response["result"]).to_equal({"expose_new": expose_new})

    # Check again if exposed - should still be False
    expect(async_should_expose(hass, "cloud.alexa", entry1.entity_id)).to_be(False)

    # Check if exposed - should be True
    expect(async_should_expose(hass, "cloud.alexa", entry2.entity_id)).to_equal(
        expose_new
    )


@test
async def listen_updates(
    hass: HomeAssistant = Depends(hass_fx),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fx),
) -> None:
    """Test listen to updates."""
    calls = []

    def listener():
        calls.append(None)

    expect(await async_setup_component(hass, "homeassistant", {})).to_be_truthy()
    await hass.async_block_till_done()

    async_listen_entity_updates(hass, "cloud.alexa", listener)

    entry = entity_registry.async_get_or_create("climate", "test", "unique1")

    # Call for another assistant - listener not called
    async_expose_entity(hass, "cloud.google_assistant", entry.entity_id, True)
    expect(len(calls)).to_equal(0)

    # Call for our assistant - listener called
    async_expose_entity(hass, "cloud.alexa", entry.entity_id, True)
    expect(len(calls)).to_equal(1)

    # Settings not changed - listener not called
    async_expose_entity(hass, "cloud.alexa", entry.entity_id, True)
    expect(len(calls)).to_equal(1)

    # Settings changed - listener called
    async_expose_entity(hass, "cloud.alexa", entry.entity_id, False)
    expect(len(calls)).to_equal(2)


@test.skip("snapshot comparison diverges under tryke - needs pytest --snapshot-update")
async def get_assistant_settings(
    hass: HomeAssistant = Depends(hass_fx),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fx),
    snap: SnapshotAssertion = Depends(snapshot),
) -> None:
    """Test get assistant settings."""
    expect(await async_setup_component(hass, "homeassistant", {})).to_be_truthy()
    await hass.async_block_till_done()

    entry = entity_registry.async_get_or_create("climate", "test", "unique1")

    expect(async_get_assistant_settings(hass, "cloud.alexa")).to_equal({})

    async_expose_entity(hass, "cloud.alexa", entry.entity_id, True)
    async_expose_entity(hass, "cloud.alexa", "light.not_in_registry", True)
    expect(async_get_assistant_settings(hass, "cloud.alexa") == snap).to_be(True)
    expect(async_get_assistant_settings(hass, "cloud.google_assistant") == snap).to_be(
        True
    )

    expect(
        lambda: async_get_entity_settings(hass, "light.unknown")
    ).to_raise(HomeAssistantError)


@test.cases(
    test.case("entities_unique_id", variant="entities_unique_id"),
    test.case("entities_no_unique_id", variant="entities_no_unique_id"),
)
async def should_expose(
    variant: str,
    hass: HomeAssistant = Depends(hass_fx),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fx),
    hass_ws_client: WebSocketGenerator = Depends(hass_ws_client_fx),
) -> None:
    """Test expose entity."""
    if variant == "entities_unique_id":
        entities = _entities_unique_id(entity_registry)
    elif variant == "entities_no_unique_id":
        entities = _entities_no_unique_id(hass)
    else:
        raise RuntimeError("Invalid setup variant")

    ws_client = await hass_ws_client(hass)
    expect(await async_setup_component(hass, "homeassistant", {})).to_be_truthy()
    await hass.async_block_till_done()

    # Expose new entities to Alexa
    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_new_entities/set",
            "assistant": "cloud.alexa",
            "expose_new": True,
        }
    )
    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()

    # Unknown entity is not exposed
    expect(async_should_expose(hass, "test.test", "test.test")).to_be(False)

    # Blocked entity is not exposed
    expect(async_should_expose(hass, "cloud.alexa", entities["blocked"])).to_be(False)

    # Lock is not exposed
    expect(async_should_expose(hass, "cloud.alexa", entities["lock"])).to_be(False)

    # Binary sensor without device class is not exposed
    expect(
        async_should_expose(hass, "cloud.alexa", entities["binary_sensor"])
    ).to_be(False)

    # Binary sensor with certain device class is exposed
    expect(
        async_should_expose(hass, "cloud.alexa", entities["door_sensor"])
    ).to_be(True)

    # Sensor without device class is not exposed
    expect(async_should_expose(hass, "cloud.alexa", entities["sensor"])).to_be(False)

    # Sensor with certain device class is exposed
    expect(
        async_should_expose(hass, "cloud.alexa", entities["temperature_sensor"])
    ).to_be(True)

    # Media player is exposed
    expect(
        async_should_expose(hass, "cloud.alexa", entities["media_player"])
    ).to_be(True)

    # The second time we check, it should load it from storage
    expect(
        async_should_expose(hass, "cloud.alexa", entities["temperature_sensor"])
    ).to_be(True)

    # Check with a different assistant
    exposed_entities = hass.data[DATA_EXPOSED_ENTITIES]
    exposed_entities.async_set_expose_new_entities("cloud.no_default_expose", False)
    expect(
        async_should_expose(
            hass, "cloud.no_default_expose", entities["temperature_sensor"]
        )
    ).to_be(False)


@test
async def should_expose_hidden_categorized(
    hass: HomeAssistant = Depends(hass_fx),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fx),
    hass_ws_client: WebSocketGenerator = Depends(hass_ws_client_fx),
) -> None:
    """Test expose entity."""
    ws_client = await hass_ws_client(hass)
    expect(await async_setup_component(hass, "homeassistant", {})).to_be_truthy()
    await hass.async_block_till_done()

    # Expose new entities to Alexa
    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_new_entities/set",
            "assistant": "cloud.alexa",
            "expose_new": True,
        }
    )
    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()

    entity_registry.async_get_or_create(
        "lock", "test", "unique2", hidden_by=er.RegistryEntryHider.USER
    )
    expect(async_should_expose(hass, "cloud.alexa", "lock.test_unique2")).to_be(False)

    # Entity with category is not exposed
    entity_registry.async_get_or_create(
        "lock", "test", "unique3", entity_category=EntityCategory.CONFIG
    )
    expect(async_should_expose(hass, "cloud.alexa", "lock.test_unique3")).to_be(False)


@test
async def list_exposed_entities(
    hass: HomeAssistant = Depends(hass_fx),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fx),
    hass_ws_client: WebSocketGenerator = Depends(hass_ws_client_fx),
) -> None:
    """Test list exposed entities."""
    ws_client = await hass_ws_client(hass)
    expect(await async_setup_component(hass, "homeassistant", {})).to_be_truthy()
    await hass.async_block_till_done()

    entry1 = entity_registry.async_get_or_create("test", "test", "unique1")
    entry2 = entity_registry.async_get_or_create("test", "test", "unique2")
    entity_registry.async_get_or_create("test", "test", "unique3")

    # Set options for registered entities
    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_entity",
            "assistants": ["cloud.alexa", "cloud.google_assistant"],
            "entity_ids": [entry1.entity_id],
            "should_expose": True,
        }
    )
    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()

    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_entity",
            "assistants": ["cloud.alexa", "cloud.google_assistant"],
            "entity_ids": [entry2.entity_id],
            "should_expose": False,
        }
    )
    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()

    # Set options for entities not in the entity registry
    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_entity",
            "assistants": ["cloud.alexa", "cloud.google_assistant"],
            "entity_ids": ["test.test"],
            "should_expose": True,
        }
    )
    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()

    await ws_client.send_json_auto_id(
        {
            "type": "homeassistant/expose_entity",
            "assistants": ["cloud.alexa", "cloud.google_assistant"],
            "entity_ids": ["test.test2"],
            "should_expose": False,
        }
    )
    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()

    # List exposed entities
    await ws_client.send_json_auto_id({"type": "homeassistant/expose_entity/list"})
    response = await ws_client.receive_json()
    expect(response["success"]).to_be_truthy()
    expect(response["result"]).to_equal(
        {
            "exposed_entities": {
                "test.test": {"cloud.alexa": True, "cloud.google_assistant": True},
                "test.test_unique1": {
                    "cloud.alexa": True,
                    "cloud.google_assistant": True,
                },
            },
        }
    )


@test
async def listeners(
    hass: HomeAssistant = Depends(hass_fx),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fx),
) -> None:
    """Make sure we call entity listeners."""
    expect(await async_setup_component(hass, "homeassistant", {})).to_be_truthy()

    exposed_entities = hass.data[DATA_EXPOSED_ENTITIES]

    callbacks = []
    exposed_entities.async_listen_entity_updates("test1", lambda: callbacks.append(1))

    async_expose_entity(hass, "test1", "light.kitchen", True)
    expect(len(callbacks)).to_equal(1)

    entry1 = entity_registry.async_get_or_create("switch", "test", "unique1")
    async_expose_entity(hass, "test1", entry1.entity_id, True)
