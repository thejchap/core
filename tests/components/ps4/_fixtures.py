"""Tryke fixtures for the PS4 integration."""
...
@fixture
def patch_io(
    _load: MagicMock = Depends(patch_load_json_object),
    _save: MagicMock = Depends(patch_save_json),
    _status: MagicMock = Depends(patch_get_status),
    _ddp: None = Depends(mock_ddp_endpoint),
) -> None:
