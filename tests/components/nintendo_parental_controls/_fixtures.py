"""Tryke fixtures for the nintendo_parental_controls integration."""
...
def mock_nintendo_client(
    device: Device = Depends(mock_nintendo_device),
    auth: MagicMock = Depends(mock_nintendo_authenticator),
) -> Generator[AsyncMock]:
    """Mock a Nintendo client."""
    mock_client_instance = AsyncMock(spec=NintendoParental)
    mock_client_instance.devices = {"testdevid": device}
    with patch(
