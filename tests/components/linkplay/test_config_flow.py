...
async def user_flow(
    ...
    _bridge: AsyncMock = Depends(mock_linkplay_factory_bridge),
...
async def zeroconf_flow_errors(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    bridge: AsyncMock = Depends(mock_linkplay_factory_bridge),
    _setup: AsyncMock = Depends(mock_setup_entry),
) -> None:
    """Test flow when the device discovered through Zeroconf cannot be reached."""
    bridge.side_effect = (LinkPlayRequestException("Error"),)
...
async def zeroconf_flow_ignores_wiim_device(
    ...
    bridge.return_value.device.manufacturer = MANUFACTURER_WIIM
...
async def zeroconf_no_probe_existing_device(
    ...
    expect(result["type"]).to_be(FlowResultType.ABORT)
    expect(result["reason"]).to_equal("already_configured")
    expect(len(bridge.mock_calls)).to_equal(0)
