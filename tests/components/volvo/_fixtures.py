"""Tryke fixtures and helpers for the Volvo tests."""
...
from tests.hass_fixtures import aioclient_mock as aioclient_mock_fx, hass as hass_fx
...
@fixture
def full_model() -> str:
    """Default model to use when running the test."""
    return DEFAULT_MODEL


@fixture
def mock_config_entry(hass: HomeAssistant = Depends(hass_fx)) -> MockConfigEntry:
...
async def setup_credentials(hass: HomeAssistant = Depends(hass_fx)) -> None:
    """Setup application credentials for the Volvo integration."""
...
async def mock_api(
    ...
    mock_api_data = await _async_load_mock_api_data(hass, model)
...
def setup_integration(
    ...
    """Return a callable that sets up the Volvo integration."""
