...
@test.cases(
    test.case("single", hosts=["1.1.1.1"]),
    test.case("subnet", hosts=["192.168.1.0/24"]),
    test.case("multi", hosts=["192.168.1.0/24", "192.168.2.0/24"]),
)
async def form(
    hosts: list[str],
    ...
    expect(CONF_SCAN_INTERVAL not in schema_defaults).to_be(True)
...
@test.cases(
    test.case("alpha", mac_excludes=["1234567890"]),
    test.case("mixed", mac_excludes=["1234567890", "11:22:33:44:55:66"]),
    test.case("nonhex", mac_excludes=["ABCDEFGHIJK"]),
)
