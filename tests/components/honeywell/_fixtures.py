"""Tryke fixtures for honeywell config flow tests."""
...
@fixture
def location(device: AsyncMock = Depends(device)) -> AsyncMock:
...
@fixture
def client(
    location: AsyncMock = Depends(location),
) -> Generator[AsyncMock]:
...
def config_entry() -> MockConfigEntry:
    ...
    return MockConfigEntry(
        domain=DOMAIN,
        data={CONF_USERNAME: "fake", CONF_PASSWORD: "user"},
        options={CONF_COOL_AWAY_TEMPERATURE: 12, CONF_HEAT_AWAY_TEMPERATURE: 22},
    )
