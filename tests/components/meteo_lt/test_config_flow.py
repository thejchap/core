...
async def api_connection_error(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    api: AsyncMock = Depends(mock_meteo_lt_api),
) -> None:
    """Test API connection error during place fetching."""
    api.places = []
    api.fetch_places.side_effect = aiohttp.ClientError("Connection failed")
...
async def api_timeout_error(
    ...
    api.places = []
    api.fetch_places.side_effect = TimeoutError("Request timed out")
