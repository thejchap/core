"""Tryke fixtures for the Leviton Decora Wi-Fi integration tests."""
...
@fixture
def mock_residence(
    mock_switch: MagicMock = Depends(mock_switch),
) -> Generator[MagicMock]:
...
def mock_decora_wifi(
    _mock_switch: MagicMock = Depends(mock_switch),
    _mock_person: MagicMock = Depends(mock_person),
    _mock_residence: MagicMock = Depends(mock_residence),
) -> Generator[MagicMock]:
