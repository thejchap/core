...
async def async_step_bluetooth_valid_device(
    ...
    expect(result2["title"]).to_equal("Triumph D36 48BE")
    expect(result2["data"]).to_equal({})
    expect(result2["result"].unique_id).to_equal("78:DB:2F:C2:48:BE")


@test
async def async_step_bluetooth_valid_io_series4_device(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    """Test discovery via bluetooth with a valid device."""
    ...
    expect(result2["type"]).to_be(FlowResultType.CREATE_ENTRY)
    expect(result2["title"]).to_equal("IO Series 4 48BE")
    expect(result2["data"]).to_equal({})
    expect(result2["result"].unique_id).to_equal("78:DB:2F:C2:48:BE")


@test
async def async_step_bluetooth_not_oralb(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
