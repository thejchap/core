...
@test.cases(
    test.case(
        ...
        info=NOT_VICTRON_SERVICE_INFO,
        expected_reason="not_supported",
    ),
    test.case(
        "bluetooth_unsupported_device",
        source=SOURCE_BLUETOOTH,
        info=VICTRON_INVERTER_RS_SERVICE_INFO,
        expected_reason="not_supported",
    ),
    test.case(
        "user_no_devices",
...
async def abort_scenarios(
    source: str,
    info: BluetoothServiceInfo | None,
    expected_reason: str,
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
...
async def async_step_user_with_found_devices_already_setup(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    _entry: MockConfigEntry = Depends(mock_config_entry_added_to_hass),
    _discovered: AsyncMock = Depends(mock_discovered_service_info),
) -> None:
    """Test setup from service info cache with devices found."""
    result = await hass.config_entries.flow.async_init(
...
async def async_step_reauth_invalid_key(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    entry: MockConfigEntry = Depends(mock_config_entry_added_to_hass),
    _discovered: AsyncMock = Depends(mock_discovered_service_info),
) -> None:
...
async def reauth_flow_sets_title_placeholders(
    ...
    expect(flows[0]["context"]["title_placeholders"]["name"]).to_equal(entry.title)
