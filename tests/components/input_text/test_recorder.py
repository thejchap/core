...
from tests.components.input_text._fixtures import (
...
async def exclude_attributes(
    ...
    now = dt_util.utcnow()
    expect(
        await async_setup_component(hass, DOMAIN, {DOMAIN: {"test": {}}})
    ).to_be_truthy()
    ...
    expect(state.attributes[ATTR_PATTERN]).to_be_none()
    ...
    for entity_states in states.values():
        for state in entity_states:
            ...
            expect(ATTR_PATTERN in state.attributes).to_be(False)
