...
@test
async def flow_user_with_api_key_v6(
...
@test
async def flow_user_with_api_key_v5(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    """Test user initialized flow with api key needed."""
...
@test
async def flow_user_invalid_v6(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
...
async def flow_reauth(
    ...
    with _patch_init_hole(mocked_hole), _patch_config_flow_hole(mocked_hole):
        expect(await hass.config_entries.async_setup(entry.entry_id)).to_be_falsy()
...
@test
async def flow_error_response(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    """Test user initialized flow but database errors occur."""
