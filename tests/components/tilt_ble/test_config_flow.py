...
async def async_step_bluetooth_valid_device(
    ...
    expect(result2["title"]).to_equal("Tilt Green")
    expect(result2["data"]).to_equal({})
    expect(result2["result"].unique_id).to_equal("F6:0F:28:F2:1F:CB")


@test
async def async_step_bluetooth_not_tilt(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
...
async def async_step_user_replaces_ignored(
    ...
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=TILT_GREEN_SERVICE_INFO.address,
