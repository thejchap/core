...
@test
async def sensor_camera_remove(
...
@test
async def sensor_sensor_remove(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    ufp: MockUFPFixture = Depends(ufp_fixture),
...
@test.skip("pre-existing failure in source pytest version (entity_id naming via translations)")
async def sensor_setup_sensor(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
...
@test.skip("pre-existing failure in source pytest version (entity_id naming via translations)")
async def sensor_setup_sensor_none(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fixture),
    ufp: MockUFPFixture = Depends(ufp_fixture),
...
async def sensor_setup_nvr(
    ...
    expected_values_d = ("50.0", "50.0", "50.0")
    for index, description in enumerate(NVR_DISABLED_SENSORS):
        ...
        state = hass.states.get(entity_id)
        assert state
        assert state.state == expected_values_d[index]
        assert state.attributes[ATTR_ATTRIBUTION] == DEFAULT_ATTRIBUTION


@test
async def sensor_nvr_missing_values(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fixture),
    ufp: MockUFPFixture = Depends(ufp_fixture),
...
async def sensor_setup_camera(
    ...
    expected_values_d = ("0.0001", "0.0001")
...
@test
async def sensor_setup_camera_with_last_trip_time(
    _trigger: int = Depends(_trigger_executor),
...
@test
async def sensor_update_alarm_with_last_trip_time(
    _trigger: int = Depends(_trigger_executor),
    _registry: None = Depends(entity_registry_enabled_by_default),
    hass: HomeAssistant = Depends(hass_fixture),
    entity_registry: er.EntityRegistry = Depends(entity_registry_fixture),
...
@test
async def aiport_no_camera_sensor_entities(
    _trigger: int = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    ufp: MockUFPFixture = Depends(ufp_fixture),
