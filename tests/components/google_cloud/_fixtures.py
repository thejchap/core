"""Tryke fixtures for Google Cloud tests."""
...
from tests.hass_fixtures import tmp_path as tmp_path_fixture
...
@fixture
def create_invalid_google_credentials_json(
    creds_path: str = Depends(create_google_credentials_json),
) -> str:
    ...
    with open(creds_path, "w", encoding="utf8") as f:
...
def mock_process_uploaded_file(
    ...
    ctx_mock = MagicMock()
    ctx_mock.__enter__.return_value = Path(creds_path)
    with patch(
...
def mock_api_tts_from_service_account_info(
    api_tts: AsyncMock = Depends(mock_api_tts),
) -> Generator[AsyncMock]:
