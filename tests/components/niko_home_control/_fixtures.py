"""Tryke fixtures for the Niko Home Control integration."""
...
def mock_niko_home_control_connection(
    light: NHCLight = Depends(light),
    dimmable_light: NHCLight = Depends(dimmable_light),
    cover: NHCCover = Depends(cover),
    climate: NHCThermostat = Depends(climate),
    scene: NHCScene = Depends(scene),
) -> Generator[AsyncMock]:
    """Mock a NHC client."""
    with (
        patch(
