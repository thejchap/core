...
from ._fixtures import host_info, os_info, supervisor_info, supervisor_root_info
...
@test
async def hassio_system_health(
    _supervisor_root_info=Depends(supervisor_root_info),
    _host_info=Depends(host_info),
    _os_info=Depends(os_info),
    _supervisor_info=Depends(supervisor_info),
    ...
    expect(await async_setup_component(hass, "system_health", {})).to_equal(True)
...
@test
async def hassio_system_health_with_issues(
    _supervisor_root_info=Depends(supervisor_root_info),
    _host_info=Depends(host_info),
    _os_info=Depends(os_info),
    _supervisor_info=Depends(supervisor_info),
    ...
    expect(info["healthy"]).to_equal({"error": "Unhealthy", "type": "failed"})
    expect(info["supported"]).to_equal({"error": "Unsupported", "type": "failed"})
    expect(info["version_api"]).to_equal({"error": "unreachable", "type": "failed"})
