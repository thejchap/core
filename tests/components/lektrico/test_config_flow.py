...
def _trigger_executor(
    _device: AsyncMock = Depends(mock_device),
...
async def user_setup(
    ...
    expect("flow_id" in result).to_be_truthy()
    ...
    expect("result" in result).to_be_truthy()
...
async def user_setup_device_offline(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    device: AsyncMock = Depends(mock_device),
) -> None:
    ...
    device.device_config.side_effect = DeviceConnectionError
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {CONF_HOST: MOCKED_DEVICE_IP_ADDRESS},
    )
    ...
    device.device_config.side_effect = None
