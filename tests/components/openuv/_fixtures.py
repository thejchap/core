"""Tryke fixtures for the OpenUV integration."""
...
def config_entry(
    ...
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=f"{cfg[CONF_LATITUDE]}, {cfg[CONF_LONGITUDE]}",
        data=cfg,
...
async def mock_pyopenuv(
    ...
    with (
        patch(
            "homeassistant.components.openuv.config_flow.Client",
            return_value=client_obj,
        ),
        patch("homeassistant.components.openuv.Client", return_value=client_obj),
    ):
        yield
