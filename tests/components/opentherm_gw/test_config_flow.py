...
from ._fixtures import mock_pyotgw, mock_setup_entry
...
async def form_user(
    ...
    pyotgw: MagicMock = Depends(mock_pyotgw),
    ...
    expect(pyotgw.return_value.connect.await_count).to_equal(1)
    expect(pyotgw.return_value.disconnect.await_count).to_equal(1)
...
async def form_connection_timeout(
    ...
    pyotgw.return_value.connect.side_effect = TimeoutError
...
async def form_connection_error(
    ...
    pyotgw.return_value.connect.side_effect = SerialException
