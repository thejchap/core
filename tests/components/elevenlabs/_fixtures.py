...
def mock_async_client() -> Generator[AsyncMock]:
    ...
    # Import tts submodule so the patch target resolves.
    import homeassistant.components.elevenlabs.tts  # noqa: F401, PLC0415
...
def mock_async_client_voices_error() -> Generator[AsyncMock]:
    """Override async ElevenLabs client with voices unauthorized ApiError."""
    client_mock = _client_mock()
    api_error = ApiError()
    api_error.body = {
        "detail": {
...
def mock_async_client_models_error() -> Generator[AsyncMock]:
    """Override async ElevenLabs client with models unauthorized ApiError."""
    client_mock = _client_mock()
    api_error = ApiError()
    api_error.body = {
        "detail": {
