...
async def logging_in_coordinator_first_update_data(
    ...
    expect("name" in coordinator.device_info).to_be(False)
    ...
    expect("Following sensors are still warming up" in caplog.text).to_be(False)
...
async def logging_in_coordinator_subsequent_update_data(
    ...
    await coordinator._async_update_data()
    expect("name" in coordinator.device_info).to_be(True)
    expect(
        "'name' not found in AirQCoordinator.device_info, fetching from the device"
        in caplog.text
    ).to_be(False)
