...
@fixture
def mock_smart_meter() -> Generator[SmartMeter]:
    """Return a Smart Meter for testing."""
    with freeze_time("2024-06-02T00:00:00Z"):
...
def mock_anglian_water_authenticator() -> Generator[MagicMock]:
    """Mock an Anglian Water authenticator."""
    with (
        patch(
...
async def mock_anglian_water_client(
    ...
    mock_smart_meter: SmartMeter = Depends(mock_smart_meter),
    mock_anglian_water_authenticator: MagicMock = Depends(mock_anglian_water_authenticator),
) -> AsyncGenerator[AsyncMock]:
    """Mock an Anglian Water client."""
    with (
        patch(
