"""Test the Energy websocket API (tryke port)."""
...
@fixture
def freeze_aug_1() -> Generator[None]:
    """Freeze time to 2021-08-01 00:00:00 UTC before fixtures resolve."""
    with freeze_time("2021-08-01 00:00:00+00:00"):
        yield


@fixture
def freeze_aug_1_one_am() -> Generator[None]:
    """Freeze time to 2021-08-01 01:00:00 UTC before fixtures resolve."""
    with freeze_time("2021-08-01 01:00:00+00:00"):
        yield


@fixture
async def _trigger_executor_aug_1(
    _freeze: None = Depends(freeze_aug_1),
    _network: None = Depends(mock_network),
    _recorder: object = Depends(recorder_mock),
) -> int:
    """Trigger executor with frozen time at 2021-08-01."""
    return 0


@fixture
async def _trigger_executor_aug_1_one_am(
    _freeze: None = Depends(freeze_aug_1_one_am),
    _network: None = Depends(mock_network),
    _recorder: object = Depends(recorder_mock),
) -> int:
    """Trigger executor with frozen time at 2021-08-01 01:00."""
    return 0


@fixture
async def setup_integration_aug_1(
    _trigger: int = Depends(_trigger_executor_aug_1),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    """Set up the energy integration with frozen time."""
    assert await async_setup_component(hass, "energy", {})


@fixture
async def setup_integration_aug_1_one_am(
    _trigger: int = Depends(_trigger_executor_aug_1_one_am),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    """Set up the energy integration with frozen time."""
    assert await async_setup_component(hass, "energy", {})
...
@test
async def get_preferences_no_data(
    ...
    expect(msg["error"]).to_equal({"code": "not_found", "message": "No prefs"})


@test
async def get_preferences_default(
    _setup: None = Depends(setup_integration),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
    ...
    expect(await is_configured(hass)).to_be_falsy()
    ...
    expect(msg["result"]).to_equal(data.EnergyManager.default_preferences())


@test
async def save_preferences(
    _setup: None = Depends(setup_integration),
    _mock_energy_platform: None = Depends(mock_energy_platform),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
    hass_storage: dict[str, Any] = Depends(hass_storage_fixture),
) -> None:
    ...
    expect(msg["id"]).to_equal(5)
    expect(msg["success"]).to_be_truthy()
    expect(msg["result"]).to_equal(default_prefs)
    ...
    expect(msg["success"]).to_be_truthy()
    expect(msg["result"]).to_equal(new_prefs)
    ...
    expect(hass_storage[data.STORAGE_KEY]["data"]).to_equal(new_prefs)

    expect(await is_configured(hass)).to_be_truthy()
    ...
    expect(msg["success"]).to_be_truthy()
    expect(msg["result"]).to_equal({**new_prefs, **new_prefs_2})


@test
async def handle_duplicate_from_stat(
    _setup: None = Depends(setup_integration),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
...
@test
async def fossil_energy_consumption_no_co2(
    _setup: None = Depends(setup_integration_aug_1),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
    ...
    if True:
        ...
        external_energy_statistics_2 = (
            {"start": period1, "last_reset": None, "state": 0, "sum": 20},
            {"start": period2, "last_reset": None, "state": 1, "sum": 30},
            {"start": period3, "last_reset": None, "state": 2, "sum": 50},
            {"start": period4, "last_reset": None, "state": 3, "sum": 80},
        )
...
@test
async def fossil_energy_consumption_hole(
    _setup: None = Depends(setup_integration_aug_1),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
    ...
    if True:
        ...
        external_energy_statistics_1 = (
            {"start": period1, "last_reset": None, "state": 0, "sum": None},
            {"start": period2, "last_reset": None, "state": 1, "sum": 3},
            {"start": period3, "last_reset": None, "state": 2, "sum": 5},
            {"start": period4, "last_reset": None, "state": 3, "sum": 8},
        )
        ...
        external_energy_statistics_2 = (
            {"start": period1, "last_reset": None, "state": 0, "sum": 20},
            {"start": period2, "last_reset": None, "state": 1, "sum": None},
            {"start": period3, "last_reset": None, "state": 2, "sum": 50},
            {"start": period4, "last_reset": None, "state": 3, "sum": 80},
        )
        external_energy_metadata_2 = {
...
@test
async def fossil_energy_consumption_no_data(
    _setup: None = Depends(setup_integration_aug_1),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
...
@test
async def fossil_energy_consumption(
    _setup: None = Depends(setup_integration_aug_1),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
    ...
    if True:
        ...
        external_energy_statistics_1 = (
            {"start": period1, "last_reset": None, "state": 0, "sum": 2},
            {"start": period2, "last_reset": None, "state": 1, "sum": 3},
            {"start": period3, "last_reset": None, "state": 2, "sum": 4},
            {"start": period4, "last_reset": None, "state": 3, "sum": 5},
        )
        external_energy_metadata_1 = {
            "has_sum": True,
            "mean_type": StatisticMeanType.NONE,
        ...
        external_energy_statistics_2 = (
            {"start": period1, "last_reset": None, "state": 0, "sum": 20000},
            {"start": period2, "last_reset": None, "state": 1, "sum": 30000},
            {"start": period3, "last_reset": None, "state": 2, "sum": 40000},
            {"start": period4, "last_reset": None, "state": 3, "sum": 50000},
        )
        external_energy_metadata_2 = {
            "has_sum": True,
            "mean_type": StatisticMeanType.NONE,
        ...
        external_co2_statistics = (
            {"start": period1, "last_reset": None, "mean": 10},
            {"start": period2, "last_reset": None, "mean": 30},
            {"start": period3, "last_reset": None, "mean": 60},
            {"start": period4, "last_reset": None, "mean": 90},
        )
...
@test
async def fossil_energy_consumption_checks(
    _setup: None = Depends(setup_integration),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
    ...
    expect(msg["success"]).to_be_falsy()
    expect(msg["error"]).to_equal(
        {"code": "invalid_end_time", "message": "Invalid end_time"}
    )


@test
async def fossil_energy_consumption_check_missing_hour(
    _setup: None = Depends(setup_integration_aug_1_one_am),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
    ...
    if True:
        ...
        energy_statistics_1 = (
            {"start": hour1, "last_reset": None, "state": 0, "sum": 1},
            {"start": hour2, "last_reset": None, "state": 1, "sum": 3},
            {"start": hour3, "last_reset": None, "state": 2, "sum": 5},
            {"start": hour4, "last_reset": None, "state": 3, "sum": 8},
        )
        energy_metadata_1 = {
            "has_sum": True,
            "mean_type": StatisticMeanType.NONE,
        ...
        co2_statistics = (
            {"start": hour1, "last_reset": None, "mean": 10},
            {"start": hour2, "last_reset": None, "mean": 30},
            {"start": hour3, "last_reset": None, "mean": 60},
            {"start": hour4, "last_reset": None, "mean": 90},
        )
        co2_metadata = {
            "has_sum": False,
            "mean_type": StatisticMeanType.ARITHMETIC,
        ...
        response = await client.receive_json()
        expect(response["success"]).to_be_truthy()
        expect(list(response["result"].keys())).to_equal(
            [
                hour1.isoformat(),
                hour2.isoformat(),
                hour3.isoformat(),
...
@test
async def fossil_energy_consumption_missing_sum(
    _setup: None = Depends(setup_integration_aug_1),
    hass: HomeAssistant = Depends(hass_fixture),
    hass_ws_client=Depends(hass_ws_client_fixture),
) -> None:
