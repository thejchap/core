...
@fixture
def mock_client(
    mock_config_obj: MagicMock = Depends(mock_config),
    mock_auth_obj: MagicMock = Depends(mock_auth),
    mock_api_obj: MagicMock = Depends(mock_api),
) -> MagicMock:
    """Return a mocked JellyfinClient."""
    jf_client = create_autospec(JellyfinClient)
    jf_client.auth = mock_auth_obj
    jf_client.config = mock_config_obj
    jf_client.jellyfin = mock_api_obj
    return jf_client


@fixture
def mock_jellyfin(
    mock_client_obj: MagicMock = Depends(mock_client),
) -> Generator[MagicMock]:
