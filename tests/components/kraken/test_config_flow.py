...
def _trigger_executor(
    ...
    _rate_limit: None = Depends(mock_call_rate_limit_sleep),
) -> None:
...
@test.skip("requires full integration setup; sensor entities not created under tryke")
async def options(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    ...
    with (
        ...
        expect(hass.states.get("sensor.xbt_usd_ask") is not None).to_be(True)
        ...
        expect(hass.states.get("sensor.xbt_usd_ask") is None).to_be(True)


@test.skip("requires full integration setup; sensor entities not created under tryke")
async def deselect_removed_pair(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
    """Test options for Kraken when a pair is removed."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        options={
            CONF_SCAN_INTERVAL: 60,
