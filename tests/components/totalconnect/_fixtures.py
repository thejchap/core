"""Tryke fixtures for TotalConnect tests."""
...
def _create_mock_zone_from_dict(zone_data: dict[str, Any]) -> AsyncMock:
    return _create_mock_zone(
        zone_data["ZoneID"],
...
@fixture
def mock_location(
    partition: AsyncMock = Depends(mock_partition),
    partition_2: AsyncMock = Depends(mock_partition_2),
    ...
    location.partitions = {1: partition, 2: partition_2}
    location.devices = {
        7654321: TotalConnectDevice(load_json_object_fixture("device_1.json", DOMAIN))
    }
    location.zones = {
        z["ZoneID"]: _create_mock_zone_from_dict(z)
...
@fixture
def mock_client(
    location: TotalConnectLocation = Depends(mock_location),
    ...
    with (
        ...
        client.locations = {location.location_id: location}
        client.usercodes = {location.location_id: CODE}
