"""The tests for Number device actions (tryke port)."""
...
async def action(
    ...
    expect(calls[0].service).to_equal("set_value")
    expect(calls[0].data).to_equal({"entity_id": entry.entity_id, "value": 0.3})
...
async def capabilities(
    ...
    expect(caps and "extra_fields" in caps).to_be_truthy()

    expect(
        voluptuous_serialize.convert(
            caps["extra_fields"], custom_serializer=cv.custom_serializer
        )
    ).to_equal([{"name": "value", "required": True, "type": "float"}])
