...
from tests.hass_fixtures import LogCapture, caplog as caplog_fx, hass as hass_fx
...
@test
async def cached_event_message(hass: HomeAssistant = Depends(hass_fx)) -> None:
...
@test
async def cached_event_message_with_different_idens(
...
@test
async def state_diff_event(hass: HomeAssistant = Depends(hass_fx)) -> None:
...
@test
async def message_to_json_bytes(caplog: LogCapture = Depends(caplog_fx)) -> None:
