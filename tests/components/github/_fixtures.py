"""Tryke fixtures for the GitHub integration."""
...
@fixture
def mock_subentries_empty() -> list[ConfigSubentryDataWithId]:
    """Fixture for empty subentries (parametrized override)."""
    return []


@fixture
def mock_config_entry(
    subentries: list[ConfigSubentryDataWithId] = Depends(mock_subentries),
) -> MockConfigEntry:
...
@fixture
def mock_config_entry_no_subentries(
    subentries: list[ConfigSubentryDataWithId] = Depends(mock_subentries_empty),
) -> MockConfigEntry:
    """Return a mocked config entry with no subentries."""
    return MockConfigEntry(
        title="",
        domain=DOMAIN,
        data={CONF_ACCESS_TOKEN: MOCK_ACCESS_TOKEN},
...
@fixture
def github_client(hass: HomeAssistant = Depends(hass_fx)) -> Generator[AsyncMock]:
    """Mock GitHub client."""
    with (
        patch(
