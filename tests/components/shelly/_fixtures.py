"""Tryke fixtures for the Shelly integration."""
...
MOCK_SETTINGS = {
    ...
    "inputs": [
        {"name": "TV LEDs", "btn_type": "momentary", "btn_reverse": 0},
        {"name": "TV Spots", "btn_type": "momentary", "btn_reverse": 0},
        {"name": None, "btn_type": "momentary", "btn_reverse": 0},
    ],
...
MOCK_STATUS_RPC = {
    "switch:0": {"id": 0, "output": True, "apower": 85.3},
...
# --- autouse-style fixtures (declare and Depends() in _trigger_executor) ---


@fixture
def mock_coap() -> Generator[None]:
    """Mock out coap."""
    with patch(
...
@fixture
def mock_ws_server() -> Generator[None]:
    """Mock out ws_server."""
...
def _mock_rpc_device(version: str | None = None) -> Mock:
...
def _mock_blu_rtv_device(version: str | None = None) -> Mock:
    """Mock rpc (Gen2, Websocket) device."""
    device = Mock(
        spec=RpcDevice,
...
@fixture
async def mock_blu_trv() -> AsyncGenerator[Mock]:
    """Mock BLU TRV (autouse-style)."""
    with (
...
@fixture
async def mock_rpc_device() -> AsyncGenerator[Mock]:
    ...
    with (
        ...
        current_pos = iter(range(50, -1, -10))

        async def update_cover_status(cover_id: int) -> None:
...
def _mock_sleepy_not_initialized_rpc_device() -> Mock:
...
def initialize_sleepy_rpc_device(device: Mock) -> None:
...
@fixture
async def mock_sleepy_rpc_device() -> AsyncGenerator[Mock]:
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with (
        ...
        patch(
            "homeassistant.components.shelly.async_unload_entry", return_value=True
        ),
    ):
...
# Export aliases for test imports
__all__ = [
    "MOCK_BLOCKS",
    "MOCK_CONFIG",
    "MOCK_MAC",
