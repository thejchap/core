"""The tests for the MQTT time platform."""
...
async def controlling_validation_state_via_topic(
    ...
    """Test the validation when controlling state via topic."""
    _ = (hass, mqtt_mock)
...
async def setting_blocked_attribute_via_mqtt_json_message(
    ...
    await help_test_setting_blocked_attribute_via_mqtt_json_message(
        hass, _make_mqtt_mock_entry(mqtt_mock), time.DOMAIN, DEFAULT_CONFIG, None
    )
...
async def discovery_update_attr(
    ...
    await help_test_discovery_update_attr(
        hass, _make_mqtt_mock_entry(mqtt_mock), time.DOMAIN, DEFAULT_CONFIG
    )
...
async def unique_id(
    ...
    """Test unique id option only creates one time per unique_id."""
    _ = (hass, mqtt_mock)
...
async def discovery_removal_time(
    ...
    """Test removal of discovered time entity."""
    data = (
        '{ "name": "test",'
    ...
    await help_test_discovery_removal(
        hass, _make_mqtt_mock_entry(mqtt_mock), time.DOMAIN, data
    )


@test
async def discovery_time_update(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test update of discovered time entity."""
    config1 = {
        "name": "Beer",
    ...
    await help_test_discovery_update(
        hass, _make_mqtt_mock_entry(mqtt_mock), time.DOMAIN, config1, config2
    )
...
async def discovery_broken(
    ...
    await help_test_discovery_broken(
        hass, _make_mqtt_mock_entry(mqtt_mock), time.DOMAIN, data1, data2
    )
...
async def entity_device_info_with_connection(
    ...
    """Test MQTT time device registry integration."""
