...
async def setting_blocked_attribute_via_mqtt_json_message(
    ...
    await help_test_setting_blocked_attribute_via_mqtt_json_message(
        hass, _make_mqtt_mock_entry(mqtt_mock), date.DOMAIN, DEFAULT_CONFIG, None
    )
...
async def discovery_update_attr(
    ...
    await help_test_discovery_update_attr(
        hass, _make_mqtt_mock_entry(mqtt_mock), date.DOMAIN, DEFAULT_CONFIG
    )
...
async def discovery_removal_time(
    ...
    await help_test_discovery_removal(
        hass, _make_mqtt_mock_entry(mqtt_mock), date.DOMAIN, data
    )
...
async def discovery_time_update(
    ...
    await help_test_discovery_update(
        hass, _make_mqtt_mock_entry(mqtt_mock), date.DOMAIN, config1, config2
    )
...
async def discovery_broken(
    ...
    await help_test_discovery_broken(
        hass, _make_mqtt_mock_entry(mqtt_mock), date.DOMAIN, data1, data2
    )
