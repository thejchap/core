...
@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def controlling_validation_state_via_topic(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test the validation of controlling state via topic."""
    _ = (hass, mqtt_mock)


@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def ambiguous_date_time_state_update(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test ambiguous date time state update."""
    _ = (hass, mqtt_mock)


@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def date_time_with_invalid_timezone_identifier(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test date time with invalid timezone identifier."""
    _ = (hass, mqtt_mock)
...
async def sending_mqtt_commands_and_optimistic(
    ...
    """Test sending MQTT commands in optimistic mode."""
    _ = (hass, mqtt_mock)
...
async def discovery_update_attr(
    ...
    await help_test_discovery_update_attr(
        hass, _make_mqtt_mock_entry(mqtt_mock), datetime.DOMAIN, DEFAULT_CONFIG
    )
...
@test
async def discovery_removal_time(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    ...
    await help_test_discovery_removal(
        hass, _make_mqtt_mock_entry(mqtt_mock), datetime.DOMAIN, data
    )


@test
async def discovery_datetime_update(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    ...
    await help_test_discovery_update(
        hass, _make_mqtt_mock_entry(mqtt_mock), datetime.DOMAIN, config1, config2
    )
...
async def discovery_broken(
    ...
    await help_test_discovery_broken(
        hass, _make_mqtt_mock_entry(mqtt_mock), datetime.DOMAIN, data1, data2
    )
...
async def entity_debug_info_message(
    ...
    await help_test_entity_debug_info_message(
        hass, _make_mqtt_mock_entry(mqtt_mock), datetime.DOMAIN, DEFAULT_CONFIG, None
    )
