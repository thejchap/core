...
@test.skip("requires hass_client_no_auth fixture not in tryke shim")
async def run_camera_setup(
...
@test.skip("requires hass_client_no_auth fixture not in tryke shim")
async def run_camera_b64_encoded(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
@test.skip("requires hass_client_no_auth fixture not in tryke shim")
async def camera_b64_encoded_with_availability(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
async def discovery_update_attr(
    ...
    await help_test_discovery_update_attr(
        hass, _make_mqtt_mock_entry(mqtt_mock), camera.DOMAIN, DEFAULT_CONFIG
    )
...
@test
async def discovery_removal_camera(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    ...
    await help_test_discovery_removal(
        hass, _make_mqtt_mock_entry(mqtt_mock), camera.DOMAIN, data
    )


@test
async def discovery_update_camera(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    ...
    await help_test_discovery_update(
        hass, _make_mqtt_mock_entry(mqtt_mock), camera.DOMAIN, config1, config2
    )


@test
async def discovery_update_unchanged_camera(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
async def discovery_broken(
    ...
    await help_test_discovery_broken(
        hass, _make_mqtt_mock_entry(mqtt_mock), camera.DOMAIN, data1, data2
    )
