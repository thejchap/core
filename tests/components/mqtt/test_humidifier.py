"""The tests for the MQTT humidifier platform."""
...
async def discovery_update_attr(
    ...
    await help_test_discovery_update_attr(
        hass, _make_mqtt_mock_entry(mqtt_mock), humidifier.DOMAIN, DEFAULT_CONFIG
    )
...
async def unique_id(
    ...
    """Test unique id option only creates one humidifier per unique_id."""
    _ = (hass, mqtt_mock)


@test
async def discovery_removal_humidifier(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    ...
    await help_test_discovery_removal(
        hass, _make_mqtt_mock_entry(mqtt_mock), humidifier.DOMAIN, data
    )


@test
async def discovery_update_humidifier(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    ...
    await help_test_discovery_update(
        hass, _make_mqtt_mock_entry(mqtt_mock), humidifier.DOMAIN, config1, config2
    )


@test
async def discovery_update_unchanged_humidifier(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
async def discovery_broken(
    ...
    await help_test_discovery_broken(
        hass, _make_mqtt_mock_entry(mqtt_mock), humidifier.DOMAIN, data1, data2
    )
...
async def entity_device_info_with_connection(
    ...
    """Test MQTT humidifier device registry integration."""
