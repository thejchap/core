...
from ._fixtures import mqtt_mock as mqtt_mock_fixture, tag_mock as tag_mock_fixture
...
@test
async def discover_bad_tag(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    device_registry: dr.DeviceRegistry = Depends(device_registry_fixture),
    tag_mock: AsyncMock = Depends(tag_mock_fixture),
) -> None:
...
@test
async def if_fires_on_mqtt_message_with_device(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    device_registry: dr.DeviceRegistry = Depends(device_registry_fixture),
    tag_mock: AsyncMock = Depends(tag_mock_fixture),
...
@test
async def if_fires_on_mqtt_message_without_device(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    tag_mock: AsyncMock = Depends(tag_mock_fixture),
) -> None:
...
@test
async def if_fires_on_mqtt_message_with_template(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    device_registry: dr.DeviceRegistry = Depends(device_registry_fixture),
    tag_mock: AsyncMock = Depends(tag_mock_fixture),
...
@test
async def if_fires_on_mqtt_message_after_update_with_device(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    device_registry: dr.DeviceRegistry = Depends(device_registry_fixture),
    tag_mock: AsyncMock = Depends(tag_mock_fixture),
...
@test
async def if_fires_on_mqtt_message_after_update_without_device(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    tag_mock: AsyncMock = Depends(tag_mock_fixture),
) -> None:
...
@test
async def if_fires_on_mqtt_message_after_update_with_template(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    device_registry: dr.DeviceRegistry = Depends(device_registry_fixture),
    tag_mock: AsyncMock = Depends(tag_mock_fixture),
...
async def no_resubscribe_same_topic(
    ...
    async_fire_mqtt_message(hass, "homeassistant/tag/bla1/config", json.dumps(config))
    await hass.async_block_till_done()
    expect(mqtt_mock.async_subscribe.call_count).to_equal(call_count)


@test
async def not_fires_on_mqtt_message_after_remove_by_mqtt_with_device(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    device_registry: dr.DeviceRegistry = Depends(device_registry_fixture),
    tag_mock: AsyncMock = Depends(tag_mock_fixture),
...
@test
async def not_fires_on_mqtt_message_after_remove_by_mqtt_without_device(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    tag_mock: AsyncMock = Depends(tag_mock_fixture),
) -> None:
...
@test
async def cleanup_device_several_tags(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    device_registry: dr.DeviceRegistry = Depends(device_registry_fixture),
    tag_mock: AsyncMock = Depends(tag_mock_fixture),
...
@test
async def cleanup_device_with_entity_and_trigger_1(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    device_registry: dr.DeviceRegistry = Depends(device_registry_fixture),
) -> None:
...
async def update_with_bad_config_not_breaks_discovery(
    ...
    await hass.async_block_till_done()
    assert "extra keys not allowed @ data['device']['bad_key']" in caplog_fixture.text
