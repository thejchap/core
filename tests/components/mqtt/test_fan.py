...
async def fail_setup_if_no_command_topic(
...
@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def controlling_state_via_topic_with_different_speed_range(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test the controlling state via topic with different speed range."""
    _ = (hass, mqtt_mock)


@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def controlling_state_via_topic_no_percentage_topics(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test the controlling state via topic without percentage state/cmd topics."""
    _ = (hass, mqtt_mock)
...
async def controlling_state_via_topic_and_json_message(
    ...
    """Test the controlling state via JSON topic."""
    _ = (hass, mqtt_mock)


@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def controlling_state_via_topic_and_json_message_shared_topic(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test the controlling state via JSON topic shared between attributes."""
    _ = (hass, mqtt_mock)
...
@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def sending_mqtt_command_templates_(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test sending command templates."""
    _ = (hass, mqtt_mock)


@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def sending_mqtt_commands_and_explicit_optimistic(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test explicit optimistic mode."""
    _ = (hass, mqtt_mock)
...
async def validity_configurations(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
async def discovery_update_attr(
    ...
    await help_test_discovery_update_attr(
        hass, _make_mqtt_mock_entry(mqtt_mock), fan.DOMAIN, DEFAULT_CONFIG
    )
...
async def unique_id(
    ...
    """Test unique id option only creates one fan per unique_id."""
    _ = (hass, mqtt_mock)


@test
async def discovery_removal_fan(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    ...
    await help_test_discovery_removal(
        hass, _make_mqtt_mock_entry(mqtt_mock), fan.DOMAIN, data
    )


@test
async def discovery_update_fan(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    ...
    await help_test_discovery_update(
        hass, _make_mqtt_mock_entry(mqtt_mock), fan.DOMAIN, config1, config2
    )


@test
async def discovery_update_unchanged_fan(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
async def discovery_broken(
    ...
    await help_test_discovery_broken(
        hass, _make_mqtt_mock_entry(mqtt_mock), fan.DOMAIN, data1, data2
    )
