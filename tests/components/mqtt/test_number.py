"""The tests for the MQTT number platform."""
...
async def run_number_setup(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test setup of number entity."""
    _ = (hass, mqtt_mock)


@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def native_value_validation(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test native value validation."""
    _ = (hass, mqtt_mock)


@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def equivalent_unit_of_measurement(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test equivalent unit of measurement."""
    _ = (hass, mqtt_mock)
...
@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def restore_native_value(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test restoring native value."""
    _ = (hass, mqtt_mock)


@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def run_number_service_optimistic(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def run_number_service_optimistic_with_command_template(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def run_number_service_with_command_template(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
async def setting_blocked_attribute_via_mqtt_json_message(
    ...
    await help_test_setting_blocked_attribute_via_mqtt_json_message(
        hass, _make_mqtt_mock_entry(mqtt_mock), number.DOMAIN, DEFAULT_CONFIG, None
    )
...
async def discovery_update_attr(
    ...
    await help_test_discovery_update_attr(
        hass, _make_mqtt_mock_entry(mqtt_mock), number.DOMAIN, DEFAULT_CONFIG
    )
...
@test
async def discovery_removal_number(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    ...
    await help_test_discovery_removal(
        hass, _make_mqtt_mock_entry(mqtt_mock), number.DOMAIN, data
    )


@test
async def discovery_update_number(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    ...
    await help_test_discovery_update(
        hass, _make_mqtt_mock_entry(mqtt_mock), number.DOMAIN, config1, config2
    )


@test
async def discovery_update_unchanged_number(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
async def discovery_broken(
    ...
    await help_test_discovery_broken(
        hass, _make_mqtt_mock_entry(mqtt_mock), number.DOMAIN, data1, data2
    )
...
@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def min_max_step_attributes(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
@test.skip("requires per-test hass_config parametrization via mqtt_mock_entry")
async def invalid_min_max_attributes(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
...
async def invalid_mode(
    ...
    """Test invalid mode attribute."""
    _ = (hass, mqtt_mock)


@test.skip("requires caplog and per-test hass_config parametrization")
async def mqtt_payload_not_a_number_warning(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test warning when MQTT payload is not a number."""
    _ = (hass, mqtt_mock)


@test.skip("requires caplog and per-test hass_config parametrization")
async def mqtt_payload_out_of_range_error(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
    mqtt_mock: Any = Depends(mqtt_mock_fixture),
) -> None:
    """Test error when MQTT payload is out of range."""
    _ = (hass, mqtt_mock)
