"""Tryke fixtures for the Notion integration."""
...
@fixture
def client(
    bridge: dict[str, Any] = Depends(data_bridge),
    listener: dict[str, Any] = Depends(data_listener),
    sensor: dict[str, Any] = Depends(data_sensor),
    user_prefs: dict[str, Any] = Depends(data_user_preferences),
) -> Mock:
    """Define a fixture for an aionotion client."""
    return Mock(
        bridge=Mock(
            async_all=AsyncMock(
                return_value=[Bridge.from_dict(b) for b in bridge["base_stations"]]
            )
        ),
        listener=Mock(
            async_all=AsyncMock(
                return_value=[Listener.from_dict(l) for l in listener["listeners"]]
            )
        ),
        refresh_token=TEST_REFRESH_TOKEN,
        sensor=Mock(
            async_all=AsyncMock(
                return_value=[Sensor.from_dict(s) for s in sensor["sensors"]]
            )
        ),
        user=Mock(
...
def config_entry(
    ...
    entry = MockConfigEntry(domain=DOMAIN, unique_id=TEST_USERNAME, data=cfg)
...
async def mock_aionotion(
    ...
    with (
        patch(
            ...
            AsyncMock(return_value=client_obj),
        ),
        patch(
