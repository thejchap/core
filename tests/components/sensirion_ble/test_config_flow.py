...
async def async_step_bluetooth_valid_device(
    ...
    expect(result2["title"]).to_equal(CONFIGURED_NAME)
    expect(result2["result"].unique_id).to_equal(SENSIRION_SERVICE_INFO.address)


@test
async def async_step_bluetooth_not_sensirion(
    _trigger: None = Depends(_trigger_executor),
    hass: HomeAssistant = Depends(hass_fixture),
) -> None:
