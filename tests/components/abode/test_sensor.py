...
# _humidity, _temperature, _illuminance — the test asserts on these.
_FAKE_TRANSLATIONS = {
...
async def attributes(
    ...
    expect(bool(state.attributes.get("battery_low"))).to_be(False)
    expect(bool(state.attributes.get("no_response"))).to_be(False)
    ...
    expect(state.attributes.get(ATTR_STATE_CLASS)).to_equal(SensorStateClass.MEASUREMENT)
    ...
    expect(abs(float(state.state) - 19.44444) < 1e-4).to_be(True)
